...
def request_paper_with_arXiv_api(keyword: str, max_results: int, link: str = "OR", sub_fileds = ["cs.PL", "cs.PF", "cs.DC", "cs.OS"]) -> List[Dict[str, str]]:
    # keyword = keyword.replace(" ", "+")
    assert link in ["OR", "AND"], "link should be 'OR' or 'AND'"
    keyword = "\"" + keyword + "\""
    if sub_fileds and len(sub_fileds) > 0:
        fileds = "cat:"
        fileds += "+OR+cat:".join(sub_fileds)
    else:
        fileds = ""
    url = "http://export.arxiv.org/api/query?search_query={3}+AND+ti:{0}+{2}+abs:{0}&max_results={1}&sortBy=lastUpdatedDate".format(keyword, max_results, link, fileds)
...
def filter_subfields(papers: List[Dict[str, str]], target_fileds: List[str]=["cs.PL", "cs.PF", "cs.DC", "cs.OS"]) -> List[Dict[str, str]]:
    # filtering tags: only keep the papers in target_fileds
    results = []
    for paper in papers:
        tags = paper.Tags
...
def get_daily_papers_by_keyword(keyword: str, column_names: List[str], max_result: int, link: str = "OR") -> List[Dict[str, str]]:
    ...
    # we only keep the papers in specific subfields
    papers = filter_subfields(papers)
